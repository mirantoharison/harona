...
import { HttpError, useNavigation, useParsed, useTranslate } from "@refinedev/core";
...
interface EstablishmentResponseProps {
  establishment: EstablishmentProps;
}

export const EstablishmentEdit = () => {
  ...
  const {
    ...
    trigger,
    control
  } = useForm<EstablishmentResponseProps, HttpError, EstablishmentProps>({
    refineCoreProps: {
  ...
  const steps = [
    translate("pages.establishment.create.stepper.coord"), 
    translate("pages.establishment.create.stepper.details"), 
    translate("pages.establishment.create.stepper.moreDetails")
  ];
  ...
  const handleRefetch = async () => {
    ...
    if (response?.data?.data?.establishment) {
      const establishment = response.data.data.establishment;
      Object.entries(establishment).forEach(([key, value]) => {
        const typedKey = key as keyof EstablishmentProps;

        if (typedKey === "types" && Array.isArray(value)) {
          setValue(typedKey, value.join(";"));
        } else {
  ...
  useEffect(() => {
    if (query?.data?.data?.establishment) {
      const data = query?.data?.data?.establishment;
      const dataKeys = Object.keys(data);

      for (const key of dataKeys) {
        const typedKey = key as keyof EstablishmentProps;
        typedKey === "types" ?
          setValue(typedKey, data[typedKey]?.join(";") ?? "") :
  ...
  return (
    <Edit
      isLoading={formLoading || query?.isFetching}
      headerProps={{
        title: (<Typography variant="h4">{translate("pages.establishment.edit.title", { id })}</Typography>),
        sx: { rowGap: "10px", flexWrap: "wrap" }
      }}
      headerButtons={({ refreshButtonProps }) => (
        <>
          <RefreshButton {...refreshButtonProps} onClick={handleRefetch}>{translate("pages.establishment.edit.headerButtons.refresh")}</RefreshButton>
